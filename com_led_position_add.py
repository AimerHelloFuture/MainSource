...
class posSpider:
    ...
    def write_lose_add(self, c):
        fp = open('lose_add_data.txt', 'a')
        fp.writelines(c + '\n')
        fp.close()

    def write_lose_add_data(self, c, name, pos, start_date, end_date):
        fp = open('lose_add_data.txt', 'a')
        fp.writelines(c + name + pos + start_date + end_date + '\n')
        fp.close()
                    ...
                    if name not in names:  # 该名字从未出现过
                        ...
                        for count in range(0, 11):
                            try:
                                ...
                                if count == 10:
                                    self.write_lose_add_data(c, name, pos, start_date, end_date)
                                    flag_r = 1
                                    break
                                else:
                                    continue
...
if __name__ == '__main__':
    ...
    fp = open('lose_led_position_data.txt', 'r')
    lose_data = []
    while True:
        line = fp.readline()
        if not line:
    ...
    for ii in range(217, len(lose_data)):
        cc = lose_data[ii]
        t_url = direct_url % (cc[0:6])
        html = requests.get(t_url)
        spider.work(cname=cc, html=html)
