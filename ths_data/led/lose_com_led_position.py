...
class ComLedPosition:
    ...
    def get_peo_uni_code(self, F003_THS029):
        ...
        if back_gro is not None and back_gro != u'':
            ...
            sql1 = 'select peo_uni_code from people_basicinfo where back_gro = %s and name = %s'
            try:
                datacenter_cursor.execute(sql1, (back_gro, name))
            except Exception, e:
                logging.error(e)
    ...
    def work(self):
        ...
        for i in range(0, len(ids)):
            ths_029_sql1 = 'select * from ths029 where F001_THS029 = %s'
            config_ths = uri.get_ths_mysql_uri()
            ths_list = uri.start_ths_MySQL(config_ths)
            ths_conn = ths_list[0]
            ths_cursor = ths_list[1]
            try:
                ths_cursor.execute(ths_029_sql1, (ids[i],))
            except Exception, e:
                logging.error(e)
