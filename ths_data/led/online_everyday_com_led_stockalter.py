...
LOG_FILENAME = "./log_everyday_com_led_stockalter.txt"
...
class GetInfo:
    ...
    def get_seq(self, people_num):
        fpp = open('led_stockalter_new_start.txt')
        try:
            seq = fpp.read()
        finally:
            fpp.close()
        ...
        fp = open('led_stockalter_new_start.txt', 'w')
        fp.writelines(str(people_num))
        fp.close()
        return seq


class ComLedStockalter:
    def __init__(self):
        self.new_num = 0
        self.info = GetInfo()
    ...
    def work(self):
        ...
        ths_cursor = ths_list[1]
        try:
            ths_cursor.execute(ths_210_sql1, (self.seq, add_num))
        except Exception, e:
            logging.error(e)
        ...
        for j in range(0, leng):
            ...
            # datacenter_conn = datacenter_list[0]
            # datacenter_cursor = datacenter_list[1]
            # sel_sql = 'select * from com_led_stockalter where com_uni_code = %s and end_date = %s and seq_sum = %s and led_name = %s and cha_name = %s'
            # try:
...
class Spider:
    ...
    def work(self):
        ...
        today = today + 'everyday_led_stockalter'
        mail_helper = MailHelper()
        try:
            mail_helper.send_mail_to(dest='ycckevinyang@163.com', message=today)
        except Exception, e:
            logging.error(e)
        everyday_led_stockalter = ComLedStockalter()
        everyday_led_stockalter.work()
...
if __name__ == '__main__':
    ...
    scheduler.add_job(spider.work, 'cron', day_of_week='0-6', hour=10, minute=33, end_date='2018-08-01')
