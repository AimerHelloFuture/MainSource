...
LOG_FILENAME = "./log_everyday_com_led_rewardstat.txt"
...
class GetInfo:
    ...
    def get_seq(self, people_num):
        fpp = open('led_rewardstat_new_start.txt')
        try:
            seq = fpp.read()
        finally:
            fpp.close()
        ...
        fp = open('led_rewardstat_new_start.txt', 'w')
...
class ComLedRewardstat:
    ...
    def work(self):
        ...
        ths_cursor = ths_list[1]
        try:
            ths_cursor.execute(ths_030_sql1, (self.seq, add_num))
        except Exception, e:
            logging.error(e)
        ...
        for j in range(0, leng):
            ...
            datacenter_conn = datacenter_list[0]
            datacenter_cursor = datacenter_list[1]
            sel_sql = 'select * from com_led_rewardstat where com_uni_code = %s and end_date = %s and peo_uni_code = %s'
            try:
                datacenter_cursor.execute(sel_sql, (com_uni_code, end_date, peo_uni_code))
            except Exception, e:
                logging.error(e)
...
class Spider:
    ...
    def work(self):
        ...
        today = today + 'everyday_led_rewardstat'
        mail_helper = MailHelper()
        try:
            mail_helper.send_mail_to(dest='ycckevinyang@163.com', message=today)
        except Exception, e:
            logging.error(e)
        everyday_led_rewardstat = ComLedRewardstat()
        everyday_led_rewardstat.work()
...
if __name__ == '__main__':
    ...
    scheduler.add_job(spider.work, 'cron', day_of_week='0-6', hour=10, minute=7, end_date='2018-08-01')
