...
class balanceSpider:
    ...
    filed_map = {
        ...
        u"证券统一编码": "sec_uni_code",
        u"人物统一编码": "peo_uni_code",
        u"变动人": "led_name",  # 即对应姓名
        u"职务": "post_code",
        u"变动前持股数": "begin_vol",
        u"变动股数": "chng_vol",
        u"变动比例(‰)": "chng_pct",
    ...
    def is_in(self, compare_date, compare_stock, compare_led, compare_vol):  # 用于判断mysql中是否存在该记录，若存在，返回True，否则返回公司统一编码
        ...
        try:
            ...
            select_sql2 = "select * from com_led_stockalter where end_date = %s and com_uni_code = %s and led_name = %s and chng_vol = %s"
    ...
    def work(self, html):
        try:
            ...
            return
        try:
            trs = soup.find("table", attrs={"class": "m-table J-ajax-table J-canvas-table"}).find('tbody').find_all('tr', recursive=False)
        except Exception, e:
            trs = None
        if not trs:
            self.write_lose()
        ...
        # 下面两个变量为了计算seq_num
        first_date = trs[0].find_all("td", recursive=False)[4].text.strip().replace('\n', '') + ' 00:00:00'  # 用于初始日期
        first_stock = trs[0].find_all("td", recursive=False)[1].text.strip().replace('\n', '')  # 用于初始代码
        seq_num = 0
        for i in range(0, len(trs)):
            tds = trs[i].find_all("td", recursive=False)
            compare_date = tds[4].text.strip().replace('\n', '') + ' 00:00:00'  # 获取日期
            compare_stock = tds[1].text.strip().replace('\n', '')  # 获取股票代码
            compare_led = tds[3].text.strip().replace('\n', '')  # 获取变动人姓名
            compare_vol = tds[5].text.strip().replace('\n', '')  # 获取变动股数
